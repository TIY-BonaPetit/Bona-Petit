...
.get(function(error, json) {
    ...
    var width = 800;
    var height = 400;
    var margins = { top: 20, right: 20, bottom: 55, left: 50 };
    ...
    //create an svg graph!
    var graphTemp = window.svg = d3.select('#temp-graph').append("svg:svg")
    .attr("width", width + margins.left + margins.right)
    .attr("height", height + margins.top + margins.bottom);

    //append x-axis
    graphTemp.append("svg:g")
    ...
    //max temp range area
    var maxRectangle = graphTemp.append("rect")
    .attr("x", 50)
    .attr("y", 55)
    .attr("width", width-70)
    .attr("height", 80)
    ...
    //min temp range area
    var minRectangle = graphTemp.append("rect")
    .attr("x", 50)
    .attr("y", 300)
    .attr("width", width-70)
    .attr("height", 80)
    ...
    var refreshTempGraph = setInterval( function() {
        ...
        .get(function(error, json) {
            ...
            var path = graphTemp.selectAll("templine").data(dataTemp);

            graphTemp.selectAll("path").remove()
            ...
            var circles = graphTemp.selectAll("circle").data(dataTemp);
    ...
    //create the graph
    var graphEC = d3.select("#ec-graph").append("svg:svg")
    .attr("width", width + margins.left + margins.right)
    .attr("height", height + margins.top + margins.bottom);

    // append x-axis
    graphEC.append("svg:g")
    .attr("class", "x axis")
    .attr("transform", "translate(0," + (height-margins.bottom/2.75) + ")")
    .call(xAxis);
    ...
    //max ec range area
    var maxRectangle = graphEC.append("rect")
    .attr("x", 50)
    .attr("y", 55)
    .attr("width", width-70)
    .attr("height", 63)
    ...
    //min temp range area
    var minRectangle = graphEC.append("rect")
    .attr("x", 50)
    .attr("y", 338)
    .attr("width", width-70)
    .attr("height", 43)
    ...
    var refreshECGraph = setInterval( function(){
        ...
        .get(function(error, json) {
            ...
            var path1 = graphEC.selectAll("path").data(dataEC);
            ...
            var circles = graphEC.selectAll("circle").data(dataEC)
